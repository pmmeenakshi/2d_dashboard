...
METRIC_COL_REGEX = re.compile(
    r"^(Tonnage|CO2_Kgs_Averted|Households_Participating|Segregation_Compliance_Pct)_(\d{4}-\d{2})$"
)
    ...
    if not has_latlon:
        ...
        for i, (comm, pin, lat, lon, city) in enumerate(zip(comm_arr, pin_arr, lat_arr, lon_arr, city_arr)):
            ...
            <div style='width:99%; margin:0 auto; text-align:left;'>
                <div style='font-size:16px; margin-bottom:5px; color:#36204D; font-weight:700;'>Tonnage</div>
                {bar_img}
            </div>
            </div>
            """
